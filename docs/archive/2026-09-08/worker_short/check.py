...
OUT=Path('/tmp/monodgp-worker-repro-OII7ks')
...
def trial(name):
    ...
    # Dataset stays full-sized; each short epoch consumes eight shuffled batches.
    ...
    manifest=dict(runtime=actual,git=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
                  ...
                  command=' '.join(sys.argv),config=cfg,initial_state=digest(model.state_dict()))
...
def compare():
    ...
    worker_checks=[]
    for w in range(4):
        states=[[json.loads((OUT/f'{run}-epoch{e}-worker{w}.json').read_text()) for e in range(2)] for run in ['A','B']]
        worker_checks.append(dict(worker=w,runs_equal=states[0]==states[1],
            epochs_different=states[0][0]['numpy_state']!=states[0][1]['numpy_state']))
    result=dict(checks=checks,workers=worker_checks,batches_per_run=16,
                samples_per_run=256,seconds=[a['seconds'],b['seconds']])
    (OUT/'result.json').write_text(json.dumps(result,indent=2))
    print(json.dumps(result,indent=2))
    assert all(checks.values()) and all(w['runs_equal'] and w['epochs_different'] for w in worker_checks)
